...
I'm building an agent that can automatically optimize its own context through MCTS-guided exploration. The core idea is that instead of hand-crafting prompts and context, the agent should be able to search through the space of possible contexts and learn and improve overtime through interactions with the user.
...
Goal is something like -> create chatnbot -> save context over time/learn what the user considers ideal/non-ideal -> do heavy reasoning with MCTS -> save context -> distill learnings into a dataset for dynamic loras (potentially)
